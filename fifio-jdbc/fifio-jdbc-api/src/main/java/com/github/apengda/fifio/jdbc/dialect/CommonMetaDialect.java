...
public class CommonMetaDialect implements DbMetaDialect {
    ...
    public List<String> buildWith(DbInfo dbInfo, String password, String tableName) {
        ...
        infos.add("'connector' = 'jdbc'");
        infos.add(String.format("'url' = '%s'", dbInfo.getUrl()));
        infos.add(String.format("'table-name' = '%s'", tableName));
        infos.add(String.format("'username' = '%s'", dbInfo.getUsername()));
        infos.add(String.format("'password' = '%s'", password));
        if (fetchSize() != null) {
            infos.add(String.format("'scan.fetch-size' = '%s'", Integer.toString(fetchSize())));
        }
        return infos;
    }
